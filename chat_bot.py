...
Room sizes: the hotel offers double bed standard rooms exclusively. The maximal number of guests for such a room is 3.
...
Price per night: about $100. An accurate price will be given at the end of the booking process."""
...
class ChatBot:
    ...
    def continue_chat(
        ...
        system_text = f"""{self.__create_instruction_prompt(state)}
        ...
        missing_information = self.__find_missing_info(state)
    ...
    def __find_missing_info(self, state: State) -> str:
    ...
    def __create_instruction_prompt(self, state: State):
        ...
        {self.__create_booking_summary_msg(state)}

        Don't discuss any topics or answer questions unrelated to the hotel booking."""

    def __create_booking_summary_msg(self, state: State) -> str:
